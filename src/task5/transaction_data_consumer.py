...
def detect_anomalies(rdd):
    if not rdd.isEmpty():
        ...
        formatted_anomalies.saveAsTextFile(f"/workspaces/Project2-cloud/output/task5/anomalies_{int(time.time())}")

def main():
    sc = SparkContext(appName="RealTimeTransactionMonitoring")
    sc.setLogLevel("ERROR")
    ssc = StreamingContext(sc, 1)  # 1 second window
    ssc.checkpoint("/workspaces/Project2-cloud/output/task5")  # Necessary for stateful transformations
    ...
    formatted_totals.foreachRDD(lambda rdd: rdd.saveAsTextFile(f"/workspaces/Project2-cloud/output/task5/running_totals_{int(time.time())}"))
