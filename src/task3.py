...
# Load Data
data = spark.read.csv('/workspaces/Project2-cloud/input/Online Retail.csv', header=True, inferSchema=True)
...
# Ensure the task3_output subdirectory exists
task3_output_dir = '/workspaces/Project2-cloud/output/task3_output/'
if not os.path.exists(task3_output_dir):
    os.mkdir(task3_output_dir)

# Writing RMSE to a file
with open(os.path.join(task3_output_dir, 'rmse.txt'), 'w') as f:
    f.write(f"Root Mean Square Error (RMSE) on test data = {rmse}\n")

# Writing MAE to a file
with open(os.path.join(task3_output_dir, 'mae.txt'), 'w') as f:
